...
import styles from './ColorBlends.module.css'
...
void main() {
  ...
  q += toward * uMouseInfluence * 0.2;
    ...
    vec3 col = vec3(0.0);
    float a = 1.0;
    float edge = 5.0; // Adjusted for better visibility (was 6.0)
    ...
    if (uColorCount > 0) {
      ...
      for (int i = 0; i < MAX_COLORS; ++i) {
            ...
            float w = 1.0 - exp(-edge / exp(edge * m));
            sumCol += uColors[i] * w;
            cover = max(cover, w);
      }
      col = clamp(sumCol * uBrightness, 0.0, 1.0);
...
export default function ColorBends({
  ...
  return <div ref={containerRef} className={styles.colorBendsContainer} style={style}/>;
}
